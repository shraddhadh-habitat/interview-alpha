...
function StatCard({ label, value, color }) {
  return (
    <div style={{ padding: '20px 24px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 10 }}>
      <div style={{ fontSize: 10, letterSpacing: 3, textTransform: 'uppercase', color: C.textMuted, fontFamily: "'DM Mono', monospace", marginBottom: 8 }}>{label}</div>
      <div style={{ fontSize: 32, fontWeight: 700, fontFamily: "'Playfair Display', serif", color: color || C.text }}>{value}</div>
    </div>
  );
}
...
export default function AdminPanel({ user }) {
  ...
  if (!user || user.email !== ADMIN_EMAIL) {
    return (
      <div style={{ minHeight: '100vh', background: C.bgSoft, paddingTop: 55, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Mono', monospace" }}>
  ...
  const statusChip = (status) => {
    ...
    return (
      <span style={{ padding: '3px 10px', background: s.bg, border: `1px solid ${s.border}`, borderRadius: 20, fontSize: 10, color: s.color, fontFamily: "'DM Mono', monospace", letterSpacing: 0.5 }}>
        {s.label}
      </span>
    );
  };
    ...
    <div style={{ minHeight: '100vh', background: C.bgSoft, paddingTop: 55, fontFamily: "'DM Mono', monospace" }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 28px', animation: 'fadeUp 0.4s cubic-bezier(0.22,1,0.36,1)' }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 32, fontWeight: 700, color: C.text, marginBottom: 4 }}>Control Panel</h1>
          ...
          {[['payments', 'Payment Requests'], ['users', 'Users']].map(([id, label]) => (
            <button
              ...
              style={{
                ...
                color: tab === id ? C.orange : C.textMuted, fontFamily: "'DM Mono', monospace",
                  ...
                  <tbody>
                    {requests.map((r, i) => (
                      <tr key={r.id} style={{ borderBottom: i < requests.length - 1 ? `1px solid ${C.border}` : 'none', background: r.status === 'pending' ? C.yellowLight : C.bg }}>
                        <td style={{ padding: '12px 14px', color: C.text, fontFamily: "'DM Mono', monospace" }}>{r.user_email}</td>
                        ...
                        <td style={{ padding: '12px 14px', color: C.text, fontFamily: "'DM Mono', monospace", letterSpacing: 0.5 }}>{r.upi_ref}</td>
                          ...
                          {r.status === 'pending' && (
                            <div style={{ display: 'flex', gap: 8 }}>
                              <button
                                ...
                                style={{ padding: '5px 12px', background: C.green, border: 'none', borderRadius: 5, color: '#fff', fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer', fontFamily: "'DM Mono', monospace", opacity: actionId === r.id ? 0.5 : 1 }}
                              >
                              ...
                              <button
                                ...
                                disabled={actionId === r.id}
                                style={{ padding: '5px 12px', background: 'transparent', border: `1px solid ${C.red}`, borderRadius: 5, color: C.red, fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', cursor: 'pointer', fontFamily: "'DM Mono', monospace" }}
                              >
                                Reject
                              </button>
                            </div>
                  ...
                  <tbody>
                    {users.map((u, i) => (
                      <tr key={u.id} style={{ borderBottom: i < users.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                        <td style={{ padding: '12px 14px', color: C.textMuted, fontFamily: "'DM Mono', monospace", fontSize: 11 }}>{u.id.slice(0, 8)}…</td>
                        ...
                        <td style={{ padding: '12px 14px', color: C.text, fontFamily: "'DM Mono', monospace" }}>{u.free_sessions_used ?? 0}</td>
                        <td style={{ padding: '12px 14px', color: C.text, fontFamily: "'DM Mono', monospace" }}>{u.monthly_sessions_used ?? 0}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {rejectTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ width: '100%', maxWidth: 440, background: C.bg, borderRadius: 16, padding: '28px 32px', boxShadow: '0 16px 48px rgba(0,0,0,0.14)' }}>
            <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 20, fontWeight: 700, color: C.text, marginBottom: 8 }}>Reject Payment</h3>
            <p style={{ fontSize: 12, color: C.textMuted, fontFamily: "'DM Mono', monospace", marginBottom: 16 }}>Optional: add a note for the user.</p>
            <textarea
              value={rejectNote}
              onChange={e => setRejectNote(e.target.value)}
              placeholder="e.g. UPI reference not found. Please resubmit."
              rows={3}
              style={{ width: '100%', padding: '10px 12px', border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 13, fontFamily: "'Source Serif 4', serif", color: C.text, resize: 'none', marginBottom: 16 }}
            />
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                onClick={handleReject}
                disabled={actionId === rejectTarget}
                style={{ flex: 1, padding: '11px 0', background: C.red, border: 'none', borderRadius: 8, color: '#fff', fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', cursor: 'pointer', fontFamily: "'DM Mono', monospace", fontWeight: 500 }}
              >
                {actionId === rejectTarget ? 'Rejecting...' : 'Confirm Reject'}
              </button>
              <button
                onClick={() => { setRejectTarget(null); setRejectNote(''); }}
                style={{ flex: 1, padding: '11px 0', background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 8, color: C.textMuted, fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', cursor: 'pointer', fontFamily: "'DM Mono', monospace" }}
              >
                Cancel
              </button>
            </div>
